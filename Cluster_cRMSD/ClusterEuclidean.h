...
class ClusterEuclidean
{
	...
	public:
		ClusterEuclidean(int, int, int, int, int, int, int, int);
		~ClusterEuclidean();
		...
		void Euclidean_Conform_Reader(std::ifstream& infile);
		double Cluster_cRMSD_Distance(Node<double>* cnode, Node<double>* inode);
		double *get_Normalized_Matrix(Node<double> *node);
};
