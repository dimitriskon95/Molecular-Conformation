...
ClusterEuclidean::ClusterEuclidean(int k_value, int dims, int hfunctions, int htables, int cfraction, int citerations, int points, int conform)
{
...
void ClusterEuclidean::ClusterEuclidean_Menu(ifstream& infile1, ifstream& infile2, ofstream& outfile)
{
	...
	Euclidean_Conform_Reader(infile1);
...
//	while(counter < 3){
	...
	/*		if (best_Silhouette < Silhouette_table[k])
			{
					best_Silhouette = Silhouette_table[k];
					best_numCluster = k;
			}		*/
	...
	//		counter++;
//	}
//	outfile << "Best Number of Cluster is " << best_numCluster << '\n';
}


//Create Conformations
void ClusterEuclidean::Euclidean_Conform_Reader(std::ifstream& infile)
{
	double *coordinate;
	Node<double>* node;
	...
	int count = 0, line_count = 0, counter=0;
	double *vector;
	...
	if (infile.is_open())
	{
			...
			{
					counter = 0;
					coordinate = new double[numPoints*3];
					...
					for(int i=0; i < numPoints; i++)
					{
							...
							tmp_vector[count] = '\0';
							coordinate[counter] = atof(tmp_vector);
							counter++;
					...
					node = new Node<double>(coordinate, 3*numPoints, conform+1, conform);
...
{
		return Cluster_cRMSD_Distance(cnode, inode);
}


double *ClusterEuclidean::get_Normalized_Matrix(Node<double>* node)
{
		double xfirstcolumn = 0, xsecondcolumn = 0, xthirdcolumn = 0;
		double *coordinate = new double[3*numPoints];
		for(int i=0; i < 3*numPoints; i++)
				coordinate[i] = node->get_data()[i];
		for(int i=0; i < numPoints; i++)
		{
				xfirstcolumn += coordinate[3*i+0];
				xsecondcolumn += coordinate[3*i+1];
				xthirdcolumn += coordinate[3*i+2];
		}
		//Metatopish (X -xc)
		xfirstcolumn = xfirstcolumn / numPoints;
		xsecondcolumn = xsecondcolumn / numPoints;
		xthirdcolumn = xthirdcolumn / numPoints;

		for(int i=0; i < numPoints; i++)
		{
				coordinate[3*i+0] = coordinate[3*i+0] - xfirstcolumn;
				coordinate[3*i+1] = coordinate[3*i+1] - xsecondcolumn;
				coordinate[3*i+2] = coordinate[3*i+2] - xthirdcolumn;
		}
...
double ClusterEuclidean::Cluster_cRMSD_Distance(Node<double>* nodeX, Node<double>* nodeY)
{
	double value;
	MatrixXd singular, Q, U, VTransposed;

	Map<MatrixXd> matrixX(get_Normalized_Matrix(nodeX), numPoints, 3);
	Map<MatrixXd> matrixY(get_Normalized_Matrix(nodeY), numPoints, 3);


	JacobiSVD<MatrixXd> svd(matrixX.transpose()*matrixY, Eigen::ComputeThinU | Eigen::ComputeThinV);
	singular = svd.singularValues();

	U = svd.matrixU();
	VTransposed = (svd.matrixV()).transpose();
	...
	value = (matrixX*Q - matrixY).norm() / sqrt(matrixX.rows());
