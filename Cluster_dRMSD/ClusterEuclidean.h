...
class ClusterEuclidean
{
	...
	public:
		ClusterEuclidean(int, int, int, int, int, int, int, int, int, int);
		~ClusterEuclidean();

		int get_ClusterNumber();
		int get_DimensionsNumber();
		void increase_ClusterNumber();
		...
		void Euclidean_Conform_Reader(std::ifstream& infile, int T);
		int **Create_DistanceID(PointerList* plist, int T);

		double Cluster_RMSD_Distance(Node<double>* cnode, Node<double>* inode);
		double Pointer_Euclidean_Distance(Pointer* cnode, Pointer* inode);
