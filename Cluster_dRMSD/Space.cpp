...
void Space::Space_Menu(ifstream& infile1, ifstream& infile2,ifstream& cfile, ofstream& outfile, int k, int r, int t)
{
	Metric_Initialization(infile1);		//Construct all classes that we need for the chosen metric

	cluster_number = 5*k;
	if (cluster_number < 5 || cluster_number > 15)
			cluster_number = 10;

	c_euclidean = new ClusterEuclidean(cluster_number, item_dimensions, hash_functions, hash_tables, clarans_franction, clarans_iterations, numPoints, numConform, r , t);
