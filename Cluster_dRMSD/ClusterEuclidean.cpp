...
ClusterEuclidean::ClusterEuclidean(int k_value, int dims, int hfunctions, int htables, int cfraction, int citerations, int points, int conform, int rvalue, int tvalue)
{
...
ClusterEuclidean::~ClusterEuclidean()
{
	...
	delete [] cluster;
	cout << "Delete Cluster Euclidean class" << endl;
}
...
void ClusterEuclidean::increase_ClusterNumber(){
	k+=5;
}
...
void ClusterEuclidean::ClusterEuclidean_Menu(ifstream& infile1, ifstream& infile2, ofstream& outfile)
{
	...
	double time, best_Silhouette = -1;
	int givenR = numPoints;
	int counter = 0;
	int T = variableT;
	...
	if (r == 0)
			...
			r = (numPoints)*(numPoints-1) / 2;

	cout << "R is " << r << endl;

/*
	cout << "Give the number of distances(r)" << endl;
	cin >> givenR;
	if (givenR >= numPoints && givenR <= (numPoints*(numPoints-1))/2 )
			r = givenR;
	else
			r = numPoints;
	cout << "You gave the value: " << r << endl;

	cout << "Give variable T (0 for min, 1 for max, 2 for random distances)." << endl;
	cin >> T;
	if (T < 0 || T > 2)
			T = 2;

	if (T == 0)
			cout << "Variable T is equal to 0 and declares " << r << " min distances." << endl;
	else if (T == 1)
			cout << "Variable T is equal to 1 and declares " << r << " max distances." << endl;
	else
			cout << "Variable T is equal to 2 and declares " << r << " random distances." << endl;
*/
	//Fisrt thing is to read all items from infile
	Euclidean_Conform_Reader(infile1, T);

	all_items_list->printList();

	cout << "Running for " << get_ClusterNumber() << " clusters." << endl;
	outfile << "k = " << k << ", R = " << r << ", T = " << variableT << '\n';

	conformList = new List<double>;
	...
	while(current != NULL)
	{
			...
			conformList->Insert_Node_End(new_node);
	...
	{
			outfile << "Cluster " << i << ": ";
			outfile << (cluster[i].get_Centroid())->get_Number() << '\t';
			current = (cluster[i].get_List())->get_start();
			while(current != NULL)
			{
					outfile << current->get_Number() << '\t';
					current = current->get_next();
			}
			outfile << '\n';
	}
	...
	delete [] cluster;
	//increase_ClusterNumber();
	cluster = new Cluster<double>[k];

	all_items_list->set_start(conformList->get_start());
	all_items_list->set_List_size(conformList->get_ListSize());

	conformList->set_start(NULL);
	delete conformList;
...
void ClusterEuclidean::Euclidean_Conform_Reader(std::ifstream& infile, int T)
{
	cout << "Reading from file..." << endl;
	string line;
	char *tmp_vector = new char[10];
	...
	Pointer *pointer;
	PointerList *list_pointers;

	if (infile.is_open())
	{
			//Take the first 2 lines
			getline(infile, line);
			getline(infile, line);

			for(int conform=0; conform < numConform; conform++)
			{
					list_pointers = new PointerList;
					...
					for(int i=0; i < numPoints; i++)
					{
							...
							{
									tmp_vector[count] = line[line_count];
									line_count++;
									count++;
							}
							tmp_vector[count] = '\0';
							coordinate = atof(tmp_vector);
							vector[0] = coordinate;
							...
							pointer = new Pointer(vector, 3, i);
							list_pointers->Insert_Pointer_End(pointer);
					}

					//create MinDistanceID or MaxDistanceID or RandomDistanceID
					int **distancesID;
					if (conform == 0 && (T == 0 || T == 1))
							distancesID = Create_DistanceID(list_pointers, T);

					//Here we will claculate distances of points
					if (T == 0 || T == 1)
					{
							distance = new double[r];

							int x = 0, y = 0;
							Pointer *pointer_one, *pointer_two, *current_point;
							for(int l=0; l < r; l++)
							{
									x = distancesID[l][0];
									y = distancesID[l][1];
									current_point = list_pointers->get_start();
									while(current_point != NULL)
									{
											if (current_point->get_id() == x)
													pointer_one = current_point;
											if (current_point->get_id() == y)
													pointer_two = current_point;

											current_point = current_point->get_next();
									}
									distance[l] = Pointer_Euclidean_Distance(pointer_one, pointer_two);
							}
					}
					else
					{
							...
							distance = new double[r];
							int counter = 0;
							Pointer *current_node_line;
							Pointer *current_node_column;
							current_node_line = list_pointers->get_start();
							while(current_node_line != NULL && counter < r)
							{
									current_node_column = current_node_line->get_next();				//we know that current_node_line != NULL
									while(current_node_column != NULL && counter < r)
									{
											distance[counter] = Pointer_Euclidean_Distance(current_node_line, current_node_column);
											counter++;
	...
	/*				cout << "For conformation "<< conform+1 << endl;
					for(int i=0; i < r; i++)
							cout << distance[i] << '\t';
					cout << endl;
	*/
					node = new Node<double>(distance, r, conform+1, conform);
					all_items_list->Insert_Node_End(node);

					//list_pointers->printList();
...
int **ClusterEuclidean::Create_DistanceID(PointerList* plist, int T)
{
		cout << "Create_DistanceID based on 1st conformation distances." << endl;
		int **id_table = new int*[r];
		for(int i=0; i < r; i++)
				id_table[i] = new int[2];


		double min, max;
		int pointer_ID_one = 0, pointer_ID_two = 0;
		Pointer *current_node_line;
		Pointer *current_node_column;

		//Here we will claculate distances of points
		int line = 0, column = 0;
		double **point_distance_table = new double*[numPoints];
		for(int i=0; i < r; i++)
				point_distance_table[i] = new double[numPoints];

		line=0;
		current_node_line = plist->get_start();
		while(current_node_line != NULL)
		{
				...
				{
					point_distance_table[line][column] = Pointer_Euclidean_Distance(current_node_line, current_node_column);
					column++;
					current_node_column = current_node_column->get_next();
				}
				line++;
		...
		if (T == 0) 								//Take min distances
		{
				//In Table point_distance_table find the minimun values and keep pointer_ID_one
				for(int dist=0; dist < r; dist++)
				{
						min = point_distance_table[0][1]; 	//Arxikopoihsh
						for(int i=0; i<= numPoints - 1; i++)
						{
								for(int j = i+1; j<= numPoints- 1; j++)
								{
										if (point_distance_table[i][j] <= min && point_distance_table[i][j] > 0)
										{
												min = point_distance_table[i][j];
												pointer_ID_one = i;
												pointer_ID_two = j;
										}
								}
						}
						point_distance_table[pointer_ID_one][pointer_ID_two] += 10000;
						id_table[dist][0] = pointer_ID_one;
						id_table[dist][1] = pointer_ID_two;
				}
		}
		else if (T == 1)						//Take max distances
		{
				//In Table point_distance_table find the minimun values and keep pointer_ID_one
				for(int dist=0; dist < r; dist++)
				{
						max = point_distance_table[0][1]; 	//Arxikopoihsh
						for(int i=0; i<= numPoints - 1; i++)
						{
								for(int j = i+1; j<= numPoints- 1; j++)
								{
										if (point_distance_table[i][j] >= max && point_distance_table[i][j] > 0)
										{
												max = point_distance_table[i][j];
												pointer_ID_one = i;
												pointer_ID_two = j;
										}
								}
						}
						point_distance_table[pointer_ID_one][pointer_ID_two] *= -1;
						id_table[dist][0] = pointer_ID_one;
						id_table[dist][1] = pointer_ID_two;
				}
		}
...
void ClusterEuclidean::Distance_Table_Calculation()
{
	...
	while(current_node_line != NULL)
	{
		...
		while(current_node_column != NULL)
		{
			Distance_Table[line][column] = Cluster_RMSD_Distance(current_node_line, current_node_column);
			column++;
			current_node_column = current_node_column->get_next();
		}
		line++;
...
double ClusterEuclidean::Pointer_Euclidean_Distance(Pointer* cnode, Pointer* inode)
{
	double dist = 0, sum = 0;
	for(int i=0; i<= inode->get_size()-1; i++)
	{
...
double ClusterEuclidean::Cluster_RMSD_Distance(Node<double>* cnode, Node<double>* inode)
{
	double dist = 0, sum = 0;
	for(int i=0; i<= inode->get_size()-1; i++)
	{
